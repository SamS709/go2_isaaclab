...
class Go2PolicyController:
    ...
    def __init__(self, policy_path, use_joint_mapping=True, policy_freq=50, control_freq=500, 
        ...
        Args:
            ...
            use_joint_mapping: If True, remap joints between SDK and policy order
        ...
        self.obs_buffer = ObservationBuffer(
            use_joint_mapping=use_joint_mapping
        )
        ...
        print(f"  Joint mapping: {use_joint_mapping}")
    ...
    def send_motor_commands(self):
        ...
        # Compute target positions (default + action * scale)
        target_positions = self.obs_buffer.default_pos + actions_sdk_order * 0.5
...
def main():
    ...
    parser.add_argument("--control-freq", type=int, default=500,
                       help="Motor command frequency in Hz (default: 500, dt=0.002)")
    parser.add_argument("--kp", type=float, default=30.0,
                       help="Position gain/stiffness (default: 25.0, use training value!)")
    parser.add_argument("--kd", type=float, default=0.5,
                       help="Velocity gain/damping (default: 0.5)")
    parser.add_argument("--action-scale", type=float, default=0.25,
                       help="Scale factor for policy actions (default: 0.25)")
    parser.add_argument("--no-mapping", action="store_true",
                       help="Disable joint order remapping")
    ...
    controller = Go2PolicyController(
        ...
        use_joint_mapping=not args.no_mapping,
